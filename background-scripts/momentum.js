...
(m.globals = m.globals || {}),
    ...
    (m.utils.oneTimeLogin = function (t) {
        return new Promise(function (n, o) {
            try {
                ...
                if (m.globals.oneTimeLoginStarted && m.globals.oneTimeLoginStarted > e - 1e4) return void o("Another login is currently being processed.");
                (m.globals.oneTimeLoginStarted = e),
                    (function (e, t, n, o) {
                        ...
                        if (r) {
                            var i = JSON.parse(r);
                            if (!i.state) return o("state passed in but no state in pendingLoginState");
                            a.pendingState = i;
                        }
                        fetch(e, { method: "post", headers: { "Content-type": "application/json; charset=UTF-8" }, body: JSON.stringify(a) })
                            .then(s)
                            .then(function (e) {
                                n(e);
                            })
                        ...
                        t,
                        function (t) {
                            if (!t || !t.token) return (m.globals.oneTimeLoginStarted = !1), void o();
                            if (
                                !(
                                    (function () {
                                        ...
                                        return n && (e = n.user_uuid || n.user_guid) ? e.toLowerCase() : null;
                                    })() !== t.user_uuid
                                )
                            )
                                return a(t), (m.globals.oneTimeLoginStarted = !1), void n();
                            m.backupDB.newBackup(localStorage.getItem("client_uuid") || new Date().getTime()).then(function () {
                                var e = localStorage.getItem("api");
                                localStorage.clear(), localStorage.clear(), e && localStorage.setItem("api", e), a(t), (m.globals.oneTimeLoginStarted = !1), n();
                            });
                        },
                        function () {
                            (m.globals.oneTimeLoginStarted = !1), o("error processing login info");
                        }
                    );
            } catch (e) {
                    ...
                    (n.onsuccess = function () {
                        ...
                        (n.onupgradeneeded = function (e) {
                            ...
                            !(function (e, t) {
                                ...
                                t.createObjectStore("backup", { keyPath: "id" }).createIndex("client", ["clientUuid"]);
                        });
                } catch (e) {}
            }
        }
    ...
    chrome.runtime.onInstalled.addListener(function (e) {
      chrome.storage.sync.set({background: {category: "macOS", wallpaperName: "Wallpaper 1", isCustom: false}})
    ...
    chrome.runtime.onMessage.addListener(function (t, n, o) {
        ...
        if ("momentum:authState" === t.type) {
            ...
            t.data.loginState ? localStorage.setItem(e, t.data.userId) : localStorage.removeItem(e);
        } else if ("checkUserId" === t.type) {
            if (!t.payload || !t.payload.userUuid) return;
            var a = localStorage.getItem("userId");
            o(a ? { isSameAccount: t.payload.userUuid === a, isLoggedIn: !0 } : { isLoggedIn: !1 });
        } else "momentum:openNew" === t.type && chrome.tabs.create({ url: "dashboard.html" });
    });
